...
def calculate(k, state_dim, init_adjacency_matrix, G, coordinates, edges, max_edge_pass, env_name, angle_limit,
              beam_num, boundary_nodes_array, heat_radius, mode, material, block, rl, random_beam, ori_center, dataset_path, rays, train_mode, dataset_mode, max_edge_length, savept):
    ...
    while not done:
        ...
        for i in range(len(beam_seq)):
            ...
            if dataset_mode and not lifting:
                new_seq = func.judge_similarity(new_adjacency_matrix, dataset_path)
        ...
        if not done:
            ...
            second_column = [row[1] for row in beam_seq]
            edge_pass += 1
